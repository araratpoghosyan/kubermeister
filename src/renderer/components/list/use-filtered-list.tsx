...
import { KIND_REGISTRY } from '../../../shared/k8s/registry';
...
export function useFilteredList<K extends Kind>(kind: K, namespace?: string) {
    ...
    // The kind's own list path names the screen a saved view belongs to.
    const screen = KIND_REGISTRY[kind].listPath;
    return {
        ...list,
        filter: (
            <>
                <LabelFilter value={selector} onChange={setSelector} />
                <ViewsMenu screen={screen} selector={selector} onApply={setSelector} />
            </>
        ),
    };
}
