...
/** The small round status indicator used in badges and the top-bar cluster-health dot. */
export function StatusDot({ tone, title, className }: { tone: StatusTone; title?: string; className?: string }) {
    return <span title={title} className={cn('size-1.5 rounded-full', DOT_TONE[tone], className)} />;
}
