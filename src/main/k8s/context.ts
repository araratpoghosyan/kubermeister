...
function toKubeContext(
    ...
    return { name, cluster, user, namespace: namespace || undefined, current: name === current };
}

export function listContexts(): KubeContext[] {
    const kc = kubeConfig();
    const current = kc.getCurrentContext();
    return kc.getContexts().map((ctx) => toKubeContext(ctx.name, ctx.cluster, ctx.user, ctx.namespace, current));
}
...
export function getCurrentContext(): KubeContext | null {
    ...
    if (!ctx) return null;
    return toKubeContext(ctx.name, ctx.cluster, ctx.user, ctx.namespace, current);
}
